...
repositories {
    mavenCentral()
    maven {
        url = uri("https://sopra-gitlab.cs.tu-dortmund.de/api/v4/projects/947/packages/maven")
        credentials(HttpHeaderCredentials::class) {
            name = "Private-Token"
            value = "glpat-R1JWLCFYiEquymoAyvMD"
        }
        authentication {
...
tasks.jacocoTestReport {
    ...
    classDirectories.setFrom(files(classDirectories.files.map {
        fileTree(it) {
            exclude(listOf("view/**", "entity/serializer.", "Main*.*", "service/ai/**"))
        }
    }))
}
