 ...
 */
class SmallGameScene(private val rootService: RootService, private val blokusApplication: BlokusApplication)
    : BoardGameScene(1920, 1080), Refreshable {
    private val unplacedBlockLength = 28
    private val placedBlockLength = 60

    private val xOffset = listOf(60, 1440)
    private val yOffset = listOf(106, 106)
    ...
    private val cornerBackground = listOf(
        Label(posX = 0, posY = 0, height = 1080, width = 540),
        Label(posX = 1380, posY = 0, height = 1080, width = 540)
    )
    ...
    private val gameBoardDropAcceptor = Label(
        ...
        dropAcceptor = {
            ...
            val coordY = (it.draggedComponent.actualPosY.toInt() - 107 + 30).floorDiv(60)
            val coordX = (it.draggedComponent.actualPosX.toInt() - 540 + 30).floorDiv(60)
            rootService.playerService.placeBlockValid(block, coordY, coordX)
        }
        onDragDropped = {
            val block = rootService.currentGameState!!.currentPlayer.blocks[selectedBlockIndex]
            val yCoord = (it.draggedComponent.actualPosY.toInt() - 107 + 30).floorDiv(60)
            val xCoord = (it.draggedComponent.actualPosX.toInt() - 540 + 30).floorDiv(60)
    ...
    private val playerBlocks = List<MutableList<Area<TokenView>>>(2){ mutableListOf() }

    private val playerNameLabels = List(2) {
        Label(
            width = 460,
            height = 70,
            font = Font(
    ...
    private val cornerGreyForeground = listOf(
        Label(posX = 0, posY = 0, height = 1080, width = 540, visual = ColorVisual(0,0,0, 80)),
        Label(posX = 1380, posY = 0, height = 1080, width = 540, visual = ColorVisual(0,0,0, 80))
    )
    ...
    private val scoreLabel = Label(
        posX = 555, posY = 15, height = 75, width = 165,
        font = Font(size = 35, color = Color.WHITE), text = "Scores: ", visual = ColorVisual.DARK_GRAY
    )
    ...
    private val saveButton = Button(
        ...
        onMouseClicked = {
            val names = rootService.currentGameState!!.players.map { "_${it.name.lowercase(Locale.getDefault())}" }.toList()
            var nameString = ""
    ...
    private val pauseButton = Button(
        ...
        if(isPaused){
            ...
            if(currentPlayer.blocks.size < nextPlayer.blocks.size || (game.players.indexOf(currentPlayer) ==
                        game.players.size -1 && currentPlayer.blocks.size == nextPlayer.blocks.size)){
    ...
    override fun refreshAfterStartGame(simulationSpeed: Int) {
        if (rootService.currentGameState!!.boardSize != 14) return
        ...
        initializePlayerBlocks(listOf(0,1))
        ...
        for(i in 0..1) {
            ...
            playerNameLabels[i].posX = (xOffset[i] -20).toDouble()
            playerNameLabels[i].posY = (15).toDouble()
        }
    ...
    override fun refreshAfterBlockPlaced(gameBoard: Array<Array<PlayerColor>>,
        ...
        for (i in 0..4){
            for(j in 0..4){
                val yCoord = coordY + i
                val xCoord = coordX + j
                if(yCoord in 0..13 && xCoord in 0..13 && gameBoard[yCoord][xCoord] != PlayerColor.BLANK){
    ...
    private fun initializePlayerBlocks(playerNumbers : List<Int>){
        ...
        for(playerNumber in playerNumbers){
            ...
            for((blockNumber, posNumber) in (0 ..20).withIndex()){
                if(givenBlockTypes.indexOfFirst { it == blockTypes[blockNumber] } >= 0){
                    playerBlocks[playerNumber].add(Area(
                        posX = xOffset[playerNumber] + (posNumber * 28 * 5) % (28 * 5 * 3),
                        posY = yOffset[playerNumber] + (posNumber / 3) * 27 * 5,
